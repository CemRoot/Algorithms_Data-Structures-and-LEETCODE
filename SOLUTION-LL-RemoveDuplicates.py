...
class LinkedList:
    ...
    def remove_duplicates(self):
        # Edge case: Empty list or single element list has no duplicates
        if self.head is None or self.head.next is None:
            return

        # Set to track values we've seen
        seen_values = set()
        
        # Start with previous as None and current at the head
        previous = None
        current = self.head
        
        # Traverse the linked list
        while current is not None:
            # If we've seen this value before, skip it by updating pointers
            if current.value in seen_values:
                # Update the previous node's next pointer to skip the current node
                previous.next = current.next
                # Decrement the length since we're removing a node
                self.length -= 1
            else:
            ...
            # Move to the next node regardless
            current = current.next
